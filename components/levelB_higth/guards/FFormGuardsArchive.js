import { PlusIcon, PencilSquareIcon, TrashIcon, ChevronDownIcon, ChevronUpIcon, RectangleGroupIcon } from '@heroicons/react/24/solid';
...
import { createGuard, editGuard, deleteGuard, recoverGuard } from '../../../src/dtos/dtoGuard';
...
import { FGuardRecoverForm } from '../../levelD_modal/guard/FGuardRecoverForm';
...
export default function FFormGuardsArchive({ accessRules, tableGuardsArchive, setTableGuards, setTableGuardsArchive, guardPosts, users }) {
  /*-------------------------------------------------------------------------------------------------------
  ...
  /*--Данные таблицы-------------------------------------------------------------------------------------*/
  const [renderTableGuards, setRenderTableGuards] = useState(tableGuardsArchive ? [...tableGuardsArchive] : []);
  ...
  const filteringTable = (text) => {
    ...
    setRenderTableGuards(
      ...
      tableGuardsArchive.filter(value => {
  ...
  const guardRecoverFormSubmit = async (event) => {
    ...
    try {
      ...
      if (MOBXuser.user && MOBXuser.user.id) {
        ...
        // Отправляем запрос на сервер
        const responce = await recoverGuard(
          guardRecoverForm.guardId
        );
        ...
        // Закрываем модальное окно
        setGuardRecoverForm({ isOpen: false });
      }

    } catch (error) {

      errorCallback(error, setGuardRecoverForm);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <FFilterText
            ...
            onClear={() => {
              setInputFilterText('');
              setRenderTableGuards([...tableGuardsArchive]);
            }}
          />
                        ...
                        <FButtonWhite
                          ...
                          onClick={(event) => {
                            ...
                            setGuardRecoverForm({
                              ...
                              guardUserPerfomed: guard.userPerfomed,
                              guardId: guard._id,
                            })
                          }}
                        >
      ...
      <FGuardRecoverForm
        form={guardRecoverForm}
        setForm={setGuardRecoverForm}
        submit={guardRecoverFormSubmit}
      />
